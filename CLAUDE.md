...
- **The synthetic request is a sub-request and is deliberately absent from the `RequestStack`.** `ControllerArgumentsEvent` is dispatched with `SUB_REQUEST` because the request only carries params. Do not "fix" this by pushing it onto the `RequestStack`: it has no URI, headers or client IP, so it would hide the real POST request from user services that inject `RequestStack`. No value resolver reads the stack — they all use the passed request.
